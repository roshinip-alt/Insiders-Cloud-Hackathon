...
# ── Excel helpers (identical logic from base.py) ─────────────────────────────
def save_history_excel(all_txns):
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Transactions"
    ws.append(["Date","Time","Merchant","Category","Txn_Type","Amount","Net Debit","Location","Session_Time"])
    for t in all_txns:
        ws.append([
...
def compute_and_save(session_txns):
    ...
    merchant_averages = {
        key: {"merchant": v["merchant"], "avg_amount": round(v["total"]/v["count"],2), "count": v["count"]}
...
def transaction():
    ...
    USER["balance"] = round(USER["balance"] - amount, 2)
    return jsonify({"transaction": txn, "balance": USER["balance"]})
